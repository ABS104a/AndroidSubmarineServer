package com.abs104a.com.client.androidsubmarine;
...
public class HTML5CamActivity extends Activity implements PreviewCallback, Callback {
    ...
    private boolean lightOldState = false;
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        ...
        if (mBoundService != null) {
            LocalHttpService.setImage(image);

            // ライト判定
            if (lightOldState != LocalHttpService.getLight()) {
                lightOldState = LocalHttpService.getLight();
                Camera.Parameters params = camera.getParameters();
                if (LocalHttpService.getLight()) {
                    //フラッシュのステータスをかえる．
